...
### Step 5：Chrome DevTools MCP 协助测试（强制）

> **强制要求**：生成故事线后，**必须**使用 **Chrome DevTools MCP** 进行测试验证。
>
> - 测试过程中发现的问题必须**自行修复**
> - 修复后需**重新测试**，直到**所有故事线节点验证通过**为止
> - **禁止**跳过测试或忽略问题
...
**必须**循环执行「测试 → 发现问题 → 自行修复 → 再测试」直到所有故事线节点通过：

```
🔄 迭代测试中...
- 第 1 轮测试：❌ 发现 2 个问题，正在自行修复...
- 第 2 轮测试：❌ 发现 1 个问题，正在自行修复...
- 第 3 轮测试：✅ 所有节点通过
```

**异常处理**：如果某个节点反复失败（超过 3 轮仍未通过），应暂停迭代，向用户确认：
...
> **重要**：完成总结输出前，必须确认所有故事线节点已通过 Chrome DevTools MCP 测试。
...
⚠️ 测试通过声明：
   - 所有故事线节点已通过 Chrome DevTools MCP 测试验证
   - 测试过程中发现的问题已全部自行修复
   - 测试报告已生成并追加至故事线文档
...
2. **Chrome DevTools MCP 测试（强制）**：
   - 故事线生成后**必须**使用 Chrome DevTools MCP 进行测试验证
   - 测试过程中发现的问题必须**自行修复**
   - 修复后必须**重新测试**，直到**所有节点通过**
   - **禁止**跳过测试或忽略未通过的问题
   - 测试完成后**必须**生成测试报告
